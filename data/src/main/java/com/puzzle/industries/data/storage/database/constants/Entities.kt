package com.puzzle.industries.data.storage.database.constants

internal object Entities {
    const val EXPENSE: String = "expense"
    const val EXPENSE_HISTORY: String = "expense_history"
    const val EXPENSE_GROUP: String = "expense_group"
    const val EXPENSE_GROUP_HISTORY: String = "expense_group_history"
    const val INCOME: String = "income"
    const val INCOME_HISTORY: String = "income_history"
    const val REMINDER: String = "reminder"
}
